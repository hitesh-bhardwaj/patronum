...
const Search = () => {
  ...
  return (
    <div
      className={`fadeUp search-container cat-btn transition-all duration-300 ease-out z-10 relative ${isSearchOpen ? "lg:mb-[9vw] mb-[20vw]" : "lg:mb-[5vw] mb-[8vw]"} `}
    >
      <button
      ...
      >
        <span>Search</span>
        <svg
          className="lg:w-[1vw] lg:h-[1vw] w-[3vw] h-[3vw]"
          width="22"
          height="22"
          viewBox="0 0 22 22"
      ...
      {/* {isSearchOpen && ( */}
      <div
        className={`absolute z-[100] lg:right-0 lg:left-auto left-0 top-full lg:mt-[1.5vw] mt-[4vw] transition-all duration-500 ease-out ${isSearchOpen ? "opacity-100 lg:w-[30vw] w-[90vw]" : "w-[0vw] opacity-0"}`}
      >
        <form
          ref={formRef}
          action="/search"
        ...
        {showSuggestions && (
          <ul className="suggestions-list bg-white rounded shadow-md lg:mt-2 mt-[2vw]">
            {results.map((result, index) => (
              ...
              >
                <Link href={postPathBySlug(result.slug)}>{result.title}</Link>
              </li>
            ))}
          </ul>
        )}
